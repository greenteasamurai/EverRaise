...
/**
 * Component that checks and displays the status of the LLM service
 */
const LLMStatusIndicator = ({ minimal = false }) => {
  const [status, setStatus] = useState('checking');

  const checkLLMStatus = async () => {
    setStatus('checking');
    try {
      const response = await fetch(getApiUrl('/api/v1/diagnostics/ollama/status'), { 
        method: 'GET',
      ...
      if (response.ok) {
        ...
        setStatus(data.available ? 'connected' : 'disconnected');
      } else {
  ...
  const getStatusText = () => {
    switch (status) {
      ...
      case 'disconnected': return 'LLM Offline';
